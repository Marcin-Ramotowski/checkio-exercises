...
class Text:
    ...
    def show(self):
        if self.font != None:
            addition = f"[{self.font}]"
            return addition + self.text + addition
        return self.text

    def restore(self, version):
        self.text, self.font = version
...
class SavedText:
    ...
    def save_text(self, Text):
        self.versions.append(Text.text)
        self.fonts.append(Text.font)

    def get_version(self, number):
        return (self.versions[number],self.fonts[number])