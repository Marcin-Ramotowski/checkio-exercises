...
def checkio(data: str):
    alphabet = 'abcdefghijklmnopqrstuwvxyzABCDEFGHIJKLMNOQPRSTUWVXYZ0123456789'
    data = ''.join([char for char in data if char in alphabet])[::-1]
    i, score = 0, 0
    for i in range(len(data)):
        ...
        if i % 2 == 0:
            points *= 2
            numbers = [points//10, points % 10]
            points = sum(numbers)
        score += points
    checkSum = str(score % 10) if score % 10 == 0 else str(10 - (score % 10))
