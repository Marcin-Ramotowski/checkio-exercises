...
def recall_password(grille: List[str], cpass: List[str]) -> str:
    matrix = np.array([list(row) for row in grille])
    cpass = np.array([list(row) for row in cpass])
    nums = [[i,j] for i, row in enumerate(matrix) for j, char in enumerate(row) if char == "X"]
    letters = [cpass[i][j]for i,j in nums]
