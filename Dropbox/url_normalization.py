...
def checkio(url):
    codes = {'%7E': '~', '%5F': '_', '%2E': '.', '%2D': '-'}
    digits = {'%3' + str(i): str(i) for i in range(0, 10)}
    alphas = {'%' + hex(i)[2:].upper(): chr(i).lower() for i in chain(range(65, 91), range(97, 123))}

    url = url.lower()  # 1
    x = re.findall('%[a-z0-9]{2}', url)  # 2
    for el in x:
        url = url.replace(el, el.upper())
    for key, char in chain(alphas.items(), digits.items(), codes.items()):  # 3
        url = url.replace(key, char)
    url = re.sub(r':80(?=/)|:80$', '', url)  # 4
    url = re.sub(r'(?<=/)\.{1}$|(?<=/)\.{1}/', '', url)  # 5
    pdir = re.search(r'/[^/]+/\.{2}$|[^/]+/\.{2}/', url)
    while pdir:
        url = url.replace(pdir.group(), '')
        pdir = re.search(r'/[^/]+/\.{2}$|[^/]+/\.{2}/', url)
    return url
