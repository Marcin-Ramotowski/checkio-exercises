...
# 
# You have to trace train tracks from "start" to "end"    in a rectangular grid, respecting constraints:Each row and column must contain a given number of tracks.Some tracks are fixed and you must use them as they are.            Their positions are given with the entry / exit directions            (North / South / West / East). You can choose the entry.The exit direction of the start,        and the entry direction of the end are fixed.The track may not cross itself.You will have to return how you move from start to end    with North / South / West / East directions.
# 
# Rows (and columns) count constraints will be represented by a list of integers.Start (and end) will be represented by its coordinates, a tuple of two integers.Fixed tracks will be represented by a dictionary            with coordinates (a tuple of two integers) as keys            and a set of entry / exit directions as values.Note:(len(rows), len(columns))give the grid dimensions.
# 
