...
# 
# Preconditions:Given puzzles are solvable        and there is only one solution (when the coordinates are sorted).7 ≤ len(grid) ≤ 50 and 7 ≤ len(grid[0]) ≤ 60.all(len(row) == len(grid[0]) for row in grid).
# 
# 
# END_DESC
