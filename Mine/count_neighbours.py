...
count_neighbours = lambda grid, x, y: [ grid[i][j] for i in range(len(grid)) for j in range(len(grid[0])) if (i >= 0 and j >= 0) and (abs(i-x)<2 and abs(j-y)<2) and (i,j)!=(x,y)].count(1)