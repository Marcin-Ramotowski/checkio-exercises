...
def checkio(number: str) -> int:
    letters = {chr(i): i-54 for i in range(65, 91)}
    digits = [str(i) for i in range(10)]
    maxChar = max(number)
    start = int(maxChar)+1 if maxChar in digits else letters[maxChar]
    for base in range(start, 37):
        n = int(number, base)  # number converted to decimal
        if n % (base-1) == 0:
            return base
    return 0